...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
    ...
    - Потому что протокол ICMP работает на сетевом уровне, а не на транспортном. Номера портов — это атрибуты транспортных протоколов, которые используются для адресации конкретных приложений на хосте. ICMP нужен для управления сетью, а не для доставки данных, поэтому порты ему не нужны.
...
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   ...
   контрольной суммы, порядкового номера и идентификатора?
    ...
    - Из скриншота выше видно, что на контрольную сумму, порядковый номер и идентификатор приходится по 2 байта.  
...
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - Видно, что аналогично предыдущему пакету.
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   с ping-запросами из Задания 1 (Ping)? Если да – то как?
    ...
    Основное отличие в поле TTL - здесь оно 1, 2, 3... Чтобы пакет доходил только до определенного маршрутизатора и умирал.
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   В нем содержится информация о пакете, который и вызвал эту ошибку. Его IP и ICMP заголовки.
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - ![img_14.png](img_14.png)
    Это обычный ответ на ping. 

   - Потому что в итоге пакет дошел до целевого хоста и вернул настоящий корректный ответ.
...
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   Видно, что с 9 до 10 прыжка задержка выросла примерно в 5-6 раз.
     
   - kbn — Копенгаген (Дания), ewr — Ньюарк (США) Именно потому, что маршрутизаторы находятся на разных континентах задержка канала так выросла.
