...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   ...
   - Всего 56 байт. Итого полезной нагрузки 56 - 20 = 36 байт.
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      компьютером (последовательность – это все сообщения, которые отправляются до конечного хоста)?
        ...
        ![img_4.png](img_4.png)
      - Видно, что меняются: идентификатор, контрольная сумма, TTL и номер в последовательности.
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Version, Header Length (20), Differentiated Services Field (0x00), Protocol (всегда ICMP = 1), Source и Destination Address, Total Length (56 — размер пакета одинаковый)

      - Version, Source и Destination Address, Protocol, Total Length точно должны не меняться для одного типа пакетов. 
        
      - Должны меняться: Identification, Header Checksum, TTL и Sequence Number
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Идентификатор всегда разный, а TTL у всех равен 63.
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      ...
      - Разный размер (последний фрагмент короче), Смещение, Флаг "More Fragments" есть в первых двух, но нет в последнем.
